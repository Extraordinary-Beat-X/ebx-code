...
static void CG_DrawSelectedPlayerWeapon( rectDef_t *rect ) {
	...
	if (pi) {
		...
		if ( cg_weapongroups[pi->curWeapon].weaponIcon ) {
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cg_weapongroups[pi->curWeapon].weaponIcon );
#else
...
#ifdef IOQ3ZTM
			CG_Draw3DHeadModel( pi - cgs.playerinfo, rect->x, rect->y, rect->w, rect->h, origin, angles );
#else
...
#ifdef TA_WEAPSYS
			if ( cg_weapongroups[pi->curWeapon].weaponIcon ) {
				CG_DrawPic( xx, y, PIC_WIDTH, PIC_WIDTH, cg_weapongroups[pi->curWeapon].weaponIcon );
			}
#else
			if ( cg_weapons[pi->curWeapon].weaponIcon ) {
