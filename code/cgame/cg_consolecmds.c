...
void CG_CamZoomIn(int localPlayerNum, qboolean down)
{
	localPlayer_t *player = &cg.localPlayers[localPlayerNum];

	player->camZoomIn = down;
	if (down) {
		player->camZoomDir -= 1.0f;
		player->camReseting = qfalse;
	}
}

void CG_CamZoomOut(int localPlayerNum, qboolean down)
{
	localPlayer_t *player = &cg.localPlayers[localPlayerNum];

	player->camZoomOut = down;
	if (down) {
		player->camZoomDir += 1.0f;
		player->camReseting = qfalse;
	}
...
void CG_CamLeft(int localPlayerNum, qboolean down)
{
	localPlayer_t *player = &cg.localPlayers[localPlayerNum];

	player->camLeft = down;
	if (down) {
		player->camRotDir += 1.0f;
		player->camReseting = qfalse;
	}
}

void CG_CamRight(int localPlayerNum, qboolean down)
{
	localPlayer_t *player = &cg.localPlayers[localPlayerNum];

	player->camRight = down;
	if (down) {
		player->camRotDir -= 1.0f;
		player->camReseting = qfalse;
	}
...
void CG_CamReset_f( int localPlayerNum )
{
	localPlayer_t *player = &cg.localPlayers[localPlayerNum];

	player->camReseting = qtrue;
	player->camZoomDir = 0;
	player->camRotDir = 0;
}
...
static playerConsoleCommand_t	playerCommands[] = {
...
#ifdef IOQ3ZTM // NEW_CAM
	{ "camreset", CG_CamReset_f, CMD_INGAME },
	{ "+camleft", CG_CamLeftDown_f, CMD_INGAME },
	{ "-camleft", CG_CamLeftUp_f, CMD_INGAME },
	{ "+camright", CG_CamRightDown_f, CMD_INGAME },
	{ "-camright", CG_CamRightUp_f, CMD_INGAME },
	{ "+camzoomin", CG_CamZoomInDown_f, CMD_INGAME },
	{ "-camzoomin", CG_CamZoomInUp_f, CMD_INGAME },
	{ "+camzoomout", CG_CamZoomOutDown_f, CMD_INGAME },
	{ "-camzoomout", CG_CamZoomOutUp_f, CMD_INGAME },
#endif
#ifdef TA_WEAPSYS_EX
	{ "+dropweapon", IN_Button1Down, 0 },
	{ "-dropweapon", IN_Button1Up, 0 },
#endif
	...
	{ "+lockon", IN_LockonDown, 0 },
	{ "-lockon", IN_LockonUp, 0 },
#endif
	...
	{ "holdable", CG_Holdable_f, CMD_INGAME },
#endif
...
#endif
#ifdef IOQ3ZTM // BLANK_HEADMODEL
	{ "player", CG_SetModel_f, 0, CG_ModelComplete },
#else
	...
	{ "dropflag", CG_ForwardToServer_f, CMD_INGAME },
#endif
};
