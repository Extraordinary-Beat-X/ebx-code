...
#ifdef TA_SP
		trap_Cvar_Register( NULL, Com_LocalPlayerCvarName(i, "spmodel"), modelNames[i], userInfo[i] | CVAR_ROM );
		trap_Cvar_Register( NULL, Com_LocalPlayerCvarName(i, "spheadmodel"), headModelNames[i], userInfo[i] | CVAR_ROM );
#endif
...
void CG_Ingame_Init( int serverMessageNum, int serverCommandSequence, int maxSplitView, int playerNum0, int playerNum1, int playerNum2, int playerNum3 ) {
	...
	for (i = 0; i < CG_MaxSplitView(); i++) {
#ifdef TA_HOLDSYS/*2*/
		cg.localPlayers[i].holdableSelect = HI_NO_SELECT;
#endif
#if defined TA_PLAYERSYS && defined TA_WEAPSYS // DEFAULT_DEFAULT_WEAPON
		// Select our default weapon.
		cg.localPlayers[i].predictedPlayerState.stats[STAT_DEFAULTWEAPON] = cgs.playerinfo[cg.localPlayers[i].playerNum].playercfg.default_weapon;
#ifdef TA_WEAPSYS_EX
		cg.localPlayers[i].predictedPlayerState.stats[STAT_PENDING_WEAPON] = cg.localPlayers[i].predictedPlayerState.stats[STAT_DEFAULTWEAPON];
#else
		cg.localPlayers[i].weaponSelect = cg.localPlayers[i].predictedPlayerState.stats[STAT_DEFAULTWEAPON];
#endif
#else
