...
#endif
			if (random() < Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_INSULT, 0, 1)) {
...
int BotChat_HitTalking(bot_state_t *bs) {
	...
	weap = BotWeaponNameForMeansOfDeath(g_entities[bs->playernum].player->lasthurt_mod,
				g_entities[bs->playernum].player->lasthurt_weapon);
	//
...
int BotChat_HitNoKill(bot_state_t *bs) {
	...
	weap = BotWeaponNameForMeansOfDeath(g_entities[bs->enemy].player->lasthurt_mod,
				g_entities[bs->enemy].player->lasthurt_weapon);
	//
